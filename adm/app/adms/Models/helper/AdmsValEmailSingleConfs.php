...
class AdmsValEmailSingleConfs
{
     ...
     * @return void   */
    public function validateEmailSingleConfs(string $email, bool|null $edit=null, int|null $id=null):void
    {
        $this->email = $email; 
        // var_dump($this->email);
        $this->edit = $edit; 
        ...
        if(($this->edit == true) and (!empty($this->id))){
            $valEmailSingle->fullRead("SELECT id FROM adms_confs_emails WHERE email =:email AND id <>:id LIMIT :limit", "email={$this->email}&id={$this->id}&limit=1");
        }else{
            $valEmailSingle->fullRead("SELECT id FROM adms_confs_emails WHERE email =:email LIMIT :limit", "email={$this->email}&limit=1");
        }
