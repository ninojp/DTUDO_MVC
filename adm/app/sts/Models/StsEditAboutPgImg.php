...
class StsEditAboutPgImg
{
    ...
    public function viewAboutPgImg(int $id):bool
    {
        ...
        if($this->resultBd){
            ...
            $_SESSION['msg'] = "<p class='alert alert-warning'>Erro (viewAboutPgImg(Models))! Imagem do Artigo não encontrado!</p>";
            $this->result = false;
            return false;
        }
    }
    ...
    public function updateAboutPg(array $data = null):void
    {
        ...
        if ($valEmptyField->getResult()) {
            if(!empty($this->dataImagem['name'])){
                ...
                $_SESSION['msg'] = "<p class='alert alert-warning'>Erro (updateAboutPg())! Necessário selecionar uma imagem</p>";
                $this->result = false;
            }
        } else {
            $this->result = false;
    ...
    private function valInput(): void
    {
        ...
        if(($this->viewAboutPgImg($this->data['id'])) and ($valExtImg->getResult())){
            // $this->result = false;
            $this->upload();
        } else {
            $this->result = false;
    ...
    private function upload()
    {
        ...
        $this->directory = "app/sts/assets/imgs/about/".$this->data['id']."/";
    ...
    * @return void     */
    private function editAboutPgImg():void
    {
        ...
        if($upUser->getResult()){
            ...
            $_SESSION['msg'] = "<p class='alert alert-warning'>Erro editAboutPgImg(models)! Imagem Não Editada com sucesso</p>";
            $this->result = false;
        }
    }
    /** =========================================================================================
    ...
    private function deleteImage():void
    {
        if(((!empty($this->resultBd[0]['image'])) or ($this->resultBd[0]['image'] != null)) and ($this->resultBd[0]['image'] != $this->nameImg)){
            $this->delImg = "app/sts/assets/imgs/about/".$this->data['id']."/".$this->resultBd[0]['image'];
