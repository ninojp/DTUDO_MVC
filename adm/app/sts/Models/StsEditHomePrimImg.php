...
/** Classe:StsEditHomePrimImg, para para editar Imagem do Premium da pagina Home */
class StsEditHomePrimImg
{
     ...
     * Método para para visualizar os dados da Imagem do Premium da pagina Home
      */
    public function viewHomePrimImg():bool
    {
        $viewHomePrimImg = new \App\adms\Models\helper\AdmsRead();
        $viewHomePrimImg->fullRead("SELECT id, prem_image FROM sts_homes_premiums LIMIT :limit", "limit=1");

        $this->resultBd = $viewHomePrimImg->getResult();
        if($this->resultBd){
            // var_dump($this->resultBd);
            $this->result = true;
            return true;
        }else{
            $_SESSION['msg'] = "<p class='alert alert-warning'>Erro (viewHomePrimImg(Models))! Imagem do Primo da pagina Home  não encontrado!</p>";
            $this->result = false;
            return false;
        }
    }
    ...
    private function valInput(): void
    {
        ...
        if(($this->viewHomePrimImg()) and ($valExtImg->getResult())){
            // $this->result = false;
            $this->upload();
        } else {
            $this->result = false;
    ...
    private function upload()
    {
        ...
        $this->directory = "app/sts/assets/imgs/home_prem/";
    ...
    * @return void     */
    private function edit():void
    {
        ...
        $upUser->exeUpdate("sts_homes_premiums", $this->data, "WHERE id=:id", "id={$this->data['id']}");
        if($upUser->getResult()){
            $this->deleteImage();
        } else {
    ...
    private function deleteImage():void
    {
        if(((!empty($this->resultBd[0]['prem_image'])) or ($this->resultBd[0]['prem_image'] != null)) and ($this->resultBd[0]['prem_image'] != $this->nameImg)){
            $this->delImg = "app/sts/assets/imgs/home_prem/".$this->resultBd[0]['prem_image'];
